...
def _fill_missing_dependencies(workbook: WorkbookData) -> Tuple[List[Dict[str, str]], PreprocessMetadata]:
    ...
    for index, row in enumerate(workbook.rows):
        ...
        predecessors_were_empty = not predecessors
        successors_were_empty = not successors
        ...
        if parent:
            ...
            if _is_valid_parent(level_number, parent_level):
                if predecessors_header and parent["can_fill_predecessors"]:
                ...
                if successors_header and successors_were_empty:
                    if parent["id"] not in successors:
                        successors.append(parent["id"])  # type: ignore[arg-type]
                        row[successors_header] = _join_ids(successors)
                        metadata.added_successors += 1
                        successors_were_empty = False
        ...
        stack.append(
            {
                ...
                "can_fill_predecessors": predecessors_were_empty,
